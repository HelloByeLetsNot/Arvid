...
async def exp(ctx):
    r = requests.get('https://game.eoserv.net/api/get_character?name=nookie')
    ...
    '       http://bbis.us/~blo/eosig/?user=nookie&type=4')
...
async def displayembed(ctx):
  ...
  browser.open("https://game.eoserv.net/character?name=Nookie")
  browser.select_form()
  browser["username"] = "torshammer"
  browser["password"] = "qwiddle1"
